...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                if (cookies.username) {
                    const response = await axios.get(`http://localhost:9090/get-data/${cookies.username}`);
    ...
    const handleDeleteTask = async (title) => {
        try {
            await axios.delete(`http://127.0.0.1:9090/delete-task/${cookies.username}/${title}`);
            setToggle(Date.now());
        } catch (error) {
            console.error(error);
        }
                        ...
                        <div className="popup bg-dark p-3 rounded">
                            ...
                            <Formik
                                ...
                                onSubmit={async (values) => {
                                    try {
                                        await axios.post(`http://127.0.0.1:9090/addtask`, values);
                                        setNewTask(false);
                                        setToggle(Date.now());
                                    } catch (error) {
                                        console.error(error);
